...
class Olympus(list):
    ...
    def key_match(self, my_string):
        pass

    def value_match(self, my_string):
        pass

    def sifter(self, key=None, value=None):
    ...
    def list_names(self):
        """ return god names """
        return [god['name'] for god in self]

    def list_attr(self):
        """ return set of attributes on all gods """
        keys = []
        [keys.extend(god.keys()) for god in self]
        return set(keys)
        ...
        for god in self:
            for key in god.keys():
                if hasattr(god[key], 'append'):
                    for val in god[key]:
                        if val != '' and val != None and key != 'description':
...
class TheOracle():
    ...
    def dispatch(self, question):
        for item in self.dispatch_table_1:
            if re.search(item, question):
                return self.dispatch_table_1[item]
    #            
    #    self.question_names = []
    #    self.question_attr = []
    #    self.question_val = []
    #    for name in self.olympus.list_names():
    #        print name
                    #if re.search(name, question):
                    #    self.question_names.append(name)
 ...
 #       try:
        first_level = False
        for item in self.dispatch_table_1:
            if re.search(item, self.question):
                self.dispatch(self.question)()
                first_level = True

        if first_level == False:
            for name in self.olympus.list_names():
                name_test = re.search(name, self.question, flags=re.I)
                if name_test != None:
                    self.question_names.append(name)

            for attr in self.olympus.list_attr():
                attr_test = re.search(attr, self.question, flags=re.I)
                if attr_test != None:
                    self.question_attr.append(attr)

            for val in self.olympus.list_val():
                val_test = re.search(val, self.question, flags=re.I)
                if val_test != None:
                    self.question_val.append(val)

        #print self.question_names
        #print self.question_attr
        #print self.question_val

        if (len(self.question_names) != 0 and len(self.question_attr) == 0 and len(self.question_val) == 0):
            for god in self.question_names:
                print self._get_god_by_name(god)['description']
        if (len(self.question_names) != 0 and len(self.question_attr) != 0):
            for god in self.question_names:
                for attr in self.question_attr:
                    try:
                        print self._get_god_by_name(god)[str(attr)]
                    except:
                        print self._get_god_by_name(god)['description']
        if (len(self.question_names) == 0 and len(self.question_attr) == 0 and len(self.question_val) != 0):
            results = []
            for val in self.question_val:
                for god in self.olympus:
                    for key in god.keys():
                        if hasattr(god[key], 'append'):
                            for entry in god[key]:
                                if entry == val:
                                    results.append(god['name'])
                        #if god[key] == val or god[key]:
                        #    results.append(god['name'])
                print "the gods of %s" %val
                for god in set(results):
                    print god
 #       except:
 #           print "I dunno"
        print "----------------------"
    ...
    def list_val(self):
        """ return set of values on all gods """
        vals = []
        [vals.extend(god.values()) for god in self.olympus] 
        return set(vals)
...
#    olympus.load(JSON_FILE)
    ...
    #print olympus.list_names()
    #print olympus.list_attr()
    #print olympus.list_val()
    while 1:
        the_oracle.prompt()
